...
class Kiwoom(QAxWidget):
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):  #주문번호 응답
        ...
        if rqname == "opt10081_req":
            ...
            for i in range(tr_data_cnt):
                ...
                self.order[code] = {
                    '종목코드': code,
                    '종목명': code_name,
                    '주문번호':order_number, #주문 접수 이후 발생되는 주문번호를 데이터베이스에 따로 저장하고, 이를 키값으로 사용하여 order 딕셔너리에서 사용하는 키 값이 유일하도록 만들어야함.
                    #그렇게 하지않으면, 분할매매할시 마지막 주문 정보만 order에 저장되기 때문에 문제 생김.
                    '주문상태': order_status,
                    '주문수량': order_quantity,
                    '주문가격': order_price,
                    '현재가': current_price,
    ...
    def send_order(self, rqname, screen_no, order_type, code, order_quantity, order_price, order_classification, origin_order_number=""):
        order_result = self.dynamicCall(
            "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)", [rqname, screen_no, self.account_number, order_type, code, order_quantity, order_price, order_classification, origin_order_number]
        )
        return order_result
    ...
    def _on_receive_real_data(self, s_code, real_type, real_data):
        if real_type == "장시작시간":
            ...
            #print(s_code, signed_at, close, high, open, low, top_priority_ask, top_priority_bid, accum_volume) 주석처리안하면 체결알림 계속 뜸
